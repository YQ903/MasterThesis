...
MatrixXd manipulabilityTrackingMainTask(Franka &robot, const MatrixXd& MDesired, vector<MatrixXd> &mLoop, vector<double> &eLoop) {
    ...
    MatrixXd Jgeo(6,7);
    // ✅ bind a reference
    const auto& m_robot = robot.getKinematicsDQ();
// or: const DQ_robotics::DQ_SerialManipulator& m_robot = robot.getKinematicsDQ();
...
MatrixXd manipulabilityTrackingSecondaryTask(
    Franka& robot, 
 ...
 */
void unitShpereTrackingMainTask(Franka& robot, 
    Eigen::Ref<const Eigen::RowVectorXd> PosInit,
