...
if args.cv is None:
    ...
    cand = [
        os.path.join(base_path, robot_teacher, dataset_map, "manipulabilities.csv"),
        os.path.join(base_path, robot_teacher, dataset_map, "manipulabilities_interpolated.csv"),
    ]
    for f in cand:
        if os.path.exists(f):
            filename_manip = f
        ...
        raise FileNotFoundError(f"None of {cand} exist")
    ...
    #manip_tmp = genfromtxt(filename_manip, delimiter=',')
    manip_tmp = np.loadtxt(filename_manip, delimiter=',')
    if manip_tmp.ndim == 1:
        manip_tmp = manip_tmp.reshape(1, -1)
