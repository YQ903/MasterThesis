...
%% Code: 
classdef RealSenseRobotTrans7DoF
    methods (Static)
        function real_sense_kinematics = kinematics()
            ...
            real_sense_DH_theta   = [0,    pi/2,   pi/2,   pi,          0,       pi/2,       0];
            real_sense_DH_d       = [0,    0,      0,      0,           0,          0,          0];
            real_sense_DH_a       = [0,    0,      0,      -0.2443,     0.2566,     0,          0];
            real_sense_DH_alpha   = [pi/2,pi/2,   pi/2,   -pi/2,         -pi/2,      -pi/2,      -pi/2];


            
            real_sense_DH_type    = repmat(DQ_SerialManipulatorMDH.JOINT_ROTATIONAL,1,7);
