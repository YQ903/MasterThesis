...
for i = 1:s
    ...
    i_jaco = i_jaco_8D([2:4, 6:8],:);
    human_mani = i_jaco * transpose(i_jaco);
    eigen_human = eig(human_mani);
    human_vol = prod(eigen_human, "all");

    % human_mani  = human_mani / human_vol * 0.006
    human_jaco_vol(i,:) = human_vol/1000;

    % human_mani  = human_mani /human_vol * 
...
plot(robot_jaco_vol)
legend('human','robot')
title('human mani volume /1000 & robot mani volume')
